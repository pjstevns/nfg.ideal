...
class idealPayment:
    ...
    def _sendRequest(self, data):
        ...
        conn.request("POST", '/xml/ideal', data,
                     {'Content-type': 'application/x-www-form-urlencoded'})
        return conn.getresponse().read()

    def createPayment(self, bank_id, amount,
                      description, return_url, report_url):
        """
        @bank_id must be a valid Bank-ID, as returned by getBanks

        @amount is a number of cents (!!) to be payed.

        @description of the transaction as reported in the bank records.
        Max-length is 30 characters (ascii).
        ...
        to check and register the payment status. This url needs to be
        reachable by mollie of course.

        separating the payment checking from the @return_url page is a security
        mechanism, not an enforced step. This means you can make it a dummy
        page, or even a non-existing page. In that case, you can and should
        check the payment status in the @return_url page, but caveat emptor!
        ...
        if self.bank_url and self.transaction_id:
            return True
        log.error("bank_url [%s] transaction_id [%s]" %
                  (self.bank_url, self.transaction_id))
        log.error("%s" % xml)
        return False
    ...
    def checkPayment(self, transaction_id):
        ...
        if self.amount and self.payed_status:
            return True
        return False
